...
const insightsDataCollector = async (reports) => {
  ...
  const avgResolutionTimeInDays = getAvgResolutionTime(reports.filter(report => report.status === "resolved"))
  ...
  const insightsData = {
    "cardsData": [
      ...
      },
      {
        ...
        line2: currentMonthReports.length !== 0 ? (changeInTimeInDays !== 0 ? `${changeInTimeInDays > 0 ? `+${changeInTimeInDays}` : changeInTimeInDays} days this month` : "No change this month") : "No change this month",

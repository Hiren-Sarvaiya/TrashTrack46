...
const getReportsSubmissionData = (currentMonthReports) => {
  const intervals = [[1, 4], [5, 8], [9, 12], [13, 16], [17, 20], [21, 24], [25, 28], [29, 31]]
    ...
    "cardsData": [
      {
        ...
        line2: currentMonthReports.length !== 0 ? `+${currentMonthReports.length} this month` : "No reports this month",
        line3: "All reports received to date."
      },
      {
        line1: "Avg. Resolution Time",
        value: `${avgResolutionTimeInDays} Days`,
        line2: changeInTimeInDays !== 0 ? `${changeInTimeInDays > 0 ? `+${changeInTimeInDays}` : changeInTimeInDays} days since last month` : "No change in resolution time this month",
        line3: "Average time to close a report."
      },
      {
        line1: "Resolved Reports",
        value: statusCounts.resolved ? statusCounts.resolved : 0,
        line2: resolvedReports.length !== 0 ? `+${resolvedReports.length} this month` : "No reports resolved this month",
        line3: "Reports successfully closed."
      },
      {
        line1: "Pending Reports",
        value: statusCounts.pending ? statusCounts.pending : 0,
        line2: pendingReportsCount !== 0 ? `+${pendingReportsCount} this month` : "No reports pending this month",
        line3: "Reports awaiting officer action."
      }
    ],
    ...
    return fetch("/api/stats")
      ...
      .then(data => {
        if (data.success) {
          ...
          console.error("Error fetching total users count : ", data.err)
          return null
        }
      })
  } catch (err) {
